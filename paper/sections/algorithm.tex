...
\begin{itemize}
  \item \textbf{LEFT ($L$)}: Value is ordered incorrectly to its left neighbor: $i > 0 \text{ and } \texttt{cmp}(A[i-1], A[i]) > 0$
  \item \textbf{RIGHT ($R$)}: Value is either already sorted correctly w.r.t its neighbors or ordered incorrectly to its right neighbor: $i = 0 \text{ or } \texttt{cmp}(A[i-1], A[i]) \le 0$
\end{itemize}
\end{definition}

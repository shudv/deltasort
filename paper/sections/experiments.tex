...
\begin{enumerate}
  ...
  \item ESM is the fastest algorithm across most of the intermediate range ($1\% \lesssim k \lesssim 70\%$). However, its performance comes at the cost of $O(n)$ space. \textbf{DeltaSort provides a space-efficient alternative to ESM for} $k \lesssim 20\%$, while still beating FullSort by $\sim 2 \times$.
...
DeltaSort was also implemented in JavaScript~\cite{deltasort-repo} and benchmarked on V8 against the native \texttt{Array.prototype.sort} to evaluate behavior in managed runtimes. The key takeaway is that DeltaSort's performance benefits rely on a \emph{predictable movement cost model}, which holds in unmanaged environments (e.g., Rust) but \textbf{not in V8~\cite{v8elementskinds}} (see~\appref{sec:appendix-js-deltasort} for benchmarks). We need further study and evaluation on other managed runtimes (e.g., JVM, .NET, JavascriptCore etc.) to understand if this is a general trend or specific to V8.
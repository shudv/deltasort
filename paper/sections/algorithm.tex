...
Several update models could be considered. For example, a \emph{bounded rank
displacement} model constrains updates such that for every updated element
$|R_{\text{new}} - R_{\text{old}}| \le b$ for some bound $b$, while a
\emph{clustered update} model restricts updated indices to a contiguous region
$[u_{\min}, u_{\max}]$ with $u_{\max} - u_{\min} \le c$. Different models interact
with DeltaSort’s structure in different ways: bounded displacement localizes
movement by design, whereas clustering can either localize repairs or increase
cross-segment interaction depending on direction patterns.

In this work, we adopt a fully random update model as a neutral baseline.
Specifically, updated indices are chosen uniformly at random from
$\{0, \ldots, n-1\}$, and updated values are drawn independently from the same
distribution as the original array. This model introduces no additional
structure that the algorithm can explicitly exploit, nor does it adversarially
bias updates toward worst-case configurations. Analyzing DeltaSort under more
structured update models, including bounded displacement and clustered updates,

...
\State $\texttt{updated} \gets \text{sort}(U)$; $\texttt{values} \gets \text{sort}([A[u] : u \in \texttt{updated}], \texttt{cmp})$
\For{$i \gets 0$ \textbf{to} $|\texttt{updated}| - 1$}: $A[\texttt{updated}[i]] \gets \texttt{values}[i]$ \EndFor
...
\For{$p \gets 0$ \textbf{to} $|\texttt{updated}| - 1$}
    \State $i \gets \texttt{updated}[p]$; $\texttt{dir} \gets (i = n)$ ? \textsc{Left} : $\Call{GetDirection}{A, i}$

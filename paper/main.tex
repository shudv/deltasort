...
\emph{RIGHT fix}: Binary search finds position $t > i$ where the element belongs. Elements in $A[i+1..t]$ shift left by one. The shift preserves relative order. By binary search, the moved element satisfies $A[t-1] < A[t] \leq A[t+1]$.
...
The graph below shows execution time (in microseconds) for $n = 50K$ elements as a function of updated count $k$. DeltaSort consistently outperforms all alternatives up to approximately $k = 16.5K$ (crossover point), achieving significant speedups of 4--20$\times$ over NativeSort in the intermediate range. Also note the orders-of-magnitude gap between DeltaSort and the baseline incremental algorithms (BIS and ESM) across the full range of $k$ values.
...
The key takeaway is that DeltaSort offers best incremental sort performance for \textbf{large} ranges of update sizes (0--25\% for the dataset we tested). The exact crossover threshold depends on the specific scenario (array size, data types, comparator cost, etc.).

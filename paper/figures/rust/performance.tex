...
\begin{axis}[
    ...
    legend style={font=\scriptsize, fill=white, fill opacity=0.95, draw=gray!50, legend columns=3, xshift=2pt, yshift=1pt},
    grid=both,
...
% Vertical line at n=100K to indicate scale of figure (a)
\addplot[black, dashed, line width=0.6pt, forget plot] coordinates {(100000, 0) (100000, 100)};

\legend{BIS, ESM, \textbf{DeltaSort}}
\end{axis}
\end{tikzpicture}
\end{minipage}
\caption{Rust benchmark results. (a)~Execution time for $n=100$K on log-log scale. (b)~Crossover threshold ($k_c/n$) after which each algorithm loses to FullSort.}

...
\begin{axis}[
    ...
    legend style={font=\scriptsize, fill=white, fill opacity=0.85, draw=gray!50, legend columns=3, xshift=-3pt, yshift=2pt, legend cell align=left, /tikz/every even column/.append style={column sep=2pt}},
...
\end{tikzpicture}
\end{minipage}
\caption{Rust benchmark results. (a)~Execution time for $n=100$K on log-log scale. (b)~Crossover threshold ($k_c/n$) after which each algorithm loses to FullSort. Refer~\appref{sec:appendix-raw-data} for raw data.}

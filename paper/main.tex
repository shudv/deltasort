...
This segmentation enables localized repair. LEFT-moving elements can be
...
contrast, RIGHT-moving elements cannot be safely placed until elements in 
the next L-segment are handled. DeltaSort therefore defers RIGHT segments
...
\begin{definition}[Violation and Direction]
\label{def:violation}
A \emph{violation} at index $i$ exists if the element at $i$ is out of order
with at least one neighbor, i.e.,
$\texttt{cmp}(A[i-1], A[i]) > 0$ (for $i > 0$) or
$\texttt{cmp}(A[i], A[i+1]) > 0$ (for $i < n-1$).

For a dirty index $i$, violations are classified by \emph{direction}:
\begin{itemize}
  \item \textbf{LEFT (L)}: $\texttt{cmp}(A[i-1], A[i]) > 0$.
  \item \textbf{RIGHT (R)}: $\texttt{cmp}(A[i], A[i+1]) > 0$.
  \item \textbf{STABLE (S)}: no violation exists at $i$.
\end{itemize}
\end{definition}

After Phase 1, dirty elements form \emph{directional segments}: contiguous groups where
all dirty elements share the same direction. This follows from Phase 1's monotonicity
property (dirty values are sorted and written to sorted indices).

\begin{lemma}[Fixing LEFT Does Not Introduce Violations]
\label{lem:fix-left}
When a LEFT dirty element at index $i$ is moved to its correct position $t < i$,
no new violations are introduced.
\end{lemma}

\begin{proof}
Consider the state when we fix a LEFT element at index $i$:

\input{figures/L-fix-invariant}

\emph{Key invariant}: At the time a LEFT element at index $i$ is processed,
the prefix $A[0..i-1]$ is sorted and contains no dirty elements with unresolved
violations.
\end{proof}

\begin{lemma}[Fixing RIGHT Does Not Introduce Violations]
\label{lem:fix-right}
When a RIGHT dirty element at index $s$ is moved to its correct position $t > s$,
no new violations are introduced.
\end{lemma}

\begin{proof}
RIGHT elements are processed from the stack in LIFO order (rightmost first).
Consider the array state when fixing a RIGHT segment:

\input{figures/R-fix-invariant}

\emph{Key invariant}: The next L-segment (to the right) is already sorted because
L-elements are fixed immediately during the scan. R-elements are processed from
the stack in LIFO order (rightmost first), so each R moves into a region bounded
by already-fixed elements. Binary search finds the correct position, elements shift
left, and the R-element slots in without introducing new violations.
\end{proof}

\begin{lemma}[All Dirty Indices Are Processed]
\label{lem:progress}
Phase 2 processes every dirty index exactly once, resolving its violation if any.
\end{lemma}

\begin{proof}
Phase 2 iterates through dirty indices in ascending order. For each index $i$:
\begin{itemize}
  \item \textbf{LEFT}: Fixed immediately via binary insertion into $A[0..i-1]$.
  \item \textbf{RIGHT}: Pushed to stack for deferred processing.
  \item \textbf{STABLE}: Pushed to stack (will be skipped when popped).
\end{itemize}
Since every dirty index is encountered during the scan and reconsidered
at most once during stack processing, and since each reconsideration
either fixes the element or leaves it STABLE, all violations are eventually resolved.
\end{proof}
...
\begin{proof}
We show that after Phase 2, $A[i] \leq A[i+1]$ for all $i$.

\emph{Base case}: After Phase 1, clean elements remain in their original sorted positions.
Violations can only occur at boundaries involving dirty elements.

\emph{Invariant}: After processing each dirty index, no new violations are introduced
(Lemmas~\ref{lem:fix-left} and~\ref{lem:fix-right}), and at least one violation is resolved.

\emph{Termination}: By Lemma~\ref{lem:progress}, all dirty indices are eventually processed.
By Lemmas~\ref{lem:fix-left} and~\ref{lem:fix-right}, each processing step
eliminates an existing violation without introducing new ones.
Since violations can only occur at dirty indices after Phase~1,
and no violations remain after all dirty indices are processed,
the array satisfies $A[i] \leq A[i+1]$ for all $i$ and is therefore sorted.
\end{proof}

...
\begin{enumerate}
  ...
  \item \textbf{RIGHT ($R$)}: Value is ordered correctly to its left neighbor or its the first value and hence \textbf{may} move right: $i = 0 \text{ or } A[i-1] \le A[i]$
\end{enumerate}
\end{definition}
...
\subparagraph{State:} We utilize the following state information during Phase~2:
\begin{itemize}
...
Under the random bounded-range update model, DeltaSort runs in $O(n\sqrt{k})$ expected time.
\end{theorem}

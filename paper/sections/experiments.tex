...
DeltaSort was also implemented in JavaScript~\cite{deltasort-repo} and benchmarked on V8 against the native \texttt{Array.prototype.sort} to evaluate behavior in managed runtimes. The key takeaways are:
\begin{enumerate}
  \item DeltaSort's performance benefits rely on a \emph{predictable movement cost model}, which holds in unmanaged environments (e.g., Rust) but \textbf{not in V8~\cite{v8elementskinds}} (see~\appref{sec:appendix-js-deltasort} for benchmarks). We need further study and evaluation on other managed runtimes (e.g., JVM, .NET, JavascriptCore etc.) to understand if this is a general trend or specific to V8.
  \item BIS and DeltaSort are $\sim$10$\times$ faster than FullSort for $k\lessapprox0.01\%$ and ESM is $\sim$1.4$\times$ faster than FullSort for $k$ up to $\approx$50\%, indicating that even in managed runtimes, exposing updated indices can unlock better performance.
\end{enumerate}

...
\draw[larrow, bend left=25] (p1.south) to node[font=\tiny, left, pos=0.3] {$-1$} (f0.north);
\draw[larrow, bend left=20] (p3.south) to node[font=\tiny, left, pos=0.3] {$-2$} (f1.north);
\draw[rarrow] (p5.south) to node[font=\tiny, right, pos=0.5] {$0$} (f5.north);
\draw[larrow, bend left=12] (p7.south) to node[font=\tiny, left, pos=0.4] {$-1$} (f6.north);
\draw[rarrow, bend right=15] (p9.south) to node[font=\tiny, right, pos=0.4] {$+1$} (f10.north);
\draw[rarrow, bend right=20] (p11.south) to node[font=\tiny, right, pos=0.4] {$+1$} (f12.north);

\end{tikzpicture}
\caption{Segmentation example: 13-element array with 6 updates at indices 1, 3, 5, 7, 9, 11. 
\textbf{Stage~1}: Original sorted array.
\textbf{Stage~2}: Updates applied (values not yet sorted among themselves).
\textbf{Stage~3}: After Phase~1, updated values are sorted and placed back at their indices; each is classified as \textcolor{blue!70!black}{L} or \textcolor{red!70!black}{R} based on violation type.
\textbf{Stage~4}: After Phase~2, each segment is repaired locally. Segment~1 contains only L-moves, Segment~2 has R then L, Segment~3 has only R-moves.}

...
As an example, consider a web application that shows a real-time game leaderboard sorted by score. Being able to efficiently update the sorted view on updates directly impacts the user experience. The traditional approaches aren't optimal have limitations: Full re-sorting is expensive for small updates, Binary-Insertion-Sort incurs high write cost ($O(kn)$) for large update batches, and Extract-Sort-Merge uses $O(n)$ auxiliary space even for small updates.

To address these limitations, this paper makes the following contributions:

\begin{enumerate}
  \item \textbf{Update-aware sorting model:}
  We formulate a sorting model in which the sorting routine is explicitly informed of the updated indices since the previous sort. Under this model, Binary-Insertion-Sort and Extract-Sort-Merge serve as the baseline algorithms because they already assume knowledge of what has been updated.
...
\begin{remark}
Note that the above model is general enough to account for additions and deletions from the array as well-
\begin{itemize}
...
\begin{definition}[Update-Aware Sort Primitive]
\label{def:sort-primitive}
The update-aware sort primitive is defined as:
\[
\textsc{Sort}(A, U, \texttt{cmp}) \rightarrow A'
\]
where:
\begin{itemize}
  \item \textbf{Input:}
  \begin{itemize}
    \item $A[0..n-1]$: an array that was sorted with respect to $\texttt{cmp}$ before updates
    \item $U \subseteq \{0, 1, \ldots, n-1\}$: the set of indices whose values have been modified
    \item $\texttt{cmp}: V \times V \rightarrow \{-1, 0, +1\}$: a comparator defining a strict weak ordering
  \end{itemize}
  \item \textbf{Output:} $A'$ is a permutation of $A$ that is sorted with respect to $\texttt{cmp}$
\end{itemize}
\end{definition}
...
\label{def:direction}
For an updated index $i$, we classify its \emph{direction} based on local order:
\begin{itemize}
...
\begin{remark}
  Note the asymmetry in the definition: a $L$ value is always an order violation while a R value might or might not be an order violation. This is deliberate. If instead, we do establish a third direction (say STABLE), it would be handled identically to how a R value is handled. Moreover, it also introduces new cases where the direction of an updated index can change from STABLE to R once a R value is fixed. Hence, this asymmetric definition simplifies the narrative and implementation by introducing a useful invariant in the system: \emph{the direction of an updated value never changes until it is fixed}.
\end{remark}
...
DeltaSort is compared against three baselines:
\begin{itemize}
    \item \textbf{NativeSort}: Rust's \texttt{sort\_by} (PDQSort~\cite{peters2021pdqsort}), representing the standard approach of re-sorting the entire array.

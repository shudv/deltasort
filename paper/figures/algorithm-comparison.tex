...
% ============================================================================
% Index labels above input array
\foreach \i in {0,...,7} {
    \node[font=\tiny, gray] at (\xI + \i*\cellw, \rowB + 0.35) {\i};
}
...
% ============================================================================
% Index labels above output array
\foreach \i in {0,...,7} {
    \node[font=\tiny, gray] at (\xO + \i*\cellw, \rowB + 0.35) {\i};
}
...
\node[black, anchor=west] at (7.25, \rowA + 0.6) {\scriptsize\bfseries Moves, Space};
\draw[arrow] (\xM + \arrayw - 0.1, \rowA) -- (8.8, \rowA) -- (8.8, \rowB) -- (\xO - \arrowgap, \rowB);
\node[black, anchor=west] at (7.8, \rowA + 0.25) {\scriptsize\bfseries 18, 0};
\draw[arrow] (\xM + \arrayw - 0.1, \rowB) -- (\xO - \arrowgap, \rowB);
\node[black, anchor=west] at (7.8, \rowB + 0.25) {\scriptsize\bfseries 15, 3};
\draw[arrow] (\xM + \arrayw - 0.1, \rowC) -- (8.8, \rowC) -- (8.8, \rowB) -- (\xO - \arrowgap, \rowB);
\node[black, anchor=west] at (7.8, \rowC + 0.25) {\scriptsize\bfseries 15, 3};
\end{tikzpicture}
\caption{Movement and space comparison for all algorithms for sorting $3$ updated values (yellow). ESM and DeltaSort are assumed to sort updated values using insertion sort to simplify movement analysis. BIS first uses $8$ moves to shift updated values to the end (since insertion requires a fully sorted prefix), then $0+4+6=10$ moves to insert each updated value into the sorted prefix. ESM uses $3+3$ moves to extract and sort the updated values, $4$ moves for compaction and then $5$ moves for backward merge. DeltaSort uses $3+3+3$ moves to collect sort and write the updated values back to the array, then just $6$ ($2+2+2$) moves to place each value correctly within its \emph{segment} (see~\defref{def:segment}), \emph{without having to form a fully sorted prefix upfront} like BIS.}

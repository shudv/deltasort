...
\For{$p \gets 0$ \textbf{to} $|\texttt{updated}| - 1$}
    ...
    \If{$\Call{GetDirection}{A, i} = \textsc{Left}$}
        \While{$\texttt{pendingRight} \neq \emptyset$} \Comment{Fix pending before LEFT}
            \State \Call{FixRightViolation}{$A$, $\texttt{pendingRight.pop}()$, $i-1$}
        \EndWhile
...
\EndFor
\Statex
\While{$\texttt{pendingRight} \neq \emptyset$} \Comment{Fix pending RIGHT violations}
    \State \Call{FixRightViolation}{$A$, $\texttt{pendingRight.pop}()$, $n-1$}
\EndWhile
\end{algorithmic}
\end{algorithm}

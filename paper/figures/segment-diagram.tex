...
\end{tikzpicture}
\caption{Segment structure after Phase~1. Each segment follows the pattern $(\mathrm{R})^*(\mathrm{L}^+ \mid \epsilon)$: zero or more RIGHT violations followed by one or more LEFT violations. Clean (unchanged) elements may be interspersed. Vertical bars denote segment boundaries. Adjacent segments $S_{i-1}$ and $S_{i+1}$ are shown faded for context.}

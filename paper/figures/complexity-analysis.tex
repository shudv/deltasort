...
\begin{axis}[
    ...
    title={\textbf{(a) Movement Complexity}},
]
...
\begin{axis}[
    ...
    major grid style={line width=0.2pt, draw=gray!50},
    legend style={font=\footnotesize, at={(0.98,0.02)}, anchor=south east},
    title={\textbf{(b) Comparison Complexity}},
]
...
\end{tikzpicture}
\end{minipage}
\caption{Empirical validation of DeltaSort's complexity bounds. Each line represents a different array size $n$. (a)~Movement normalized by $n\sqrt{k}$ converges to $\approx 0.4$, confirming $O(n\sqrt{k})$. (b)~Comparisons normalized by $k\log n$ converges to $\approx 2$, confirming $O(k\log n)$. The overlap of lines across scales validates the asymptotic analysis.}

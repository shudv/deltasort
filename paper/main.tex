...
\begin{enumerate}
    ...
    \item \textbf{Binary-Insertion-Sort (BIS)}: Space efficient ($O(1)$), but movement intensive ($O(kn)$).

    \item \textbf{Extract-Sort-Merge (ESM)}: Movement efficient ($O(n)$), but requires additional space ($O(n)$) for all $k$.
\end{enumerate}

...
\draw[movearrow] ([xshift=-0.1cm, yshift=0.02cm]bisM5.north) 
    .. controls ++(0, 0.4) and ++(0, 0.4) .. 
    ([xshift=0.1cm, yshift=0.02cm]bisM5.north) node[above, movelabel, yshift=0.15cm, xshift=-0.1cm] {0};
% Arrow for 4: goes above to avoid overlap with 6
\draw[movearrow] ([yshift=0.25cm]bisM6.north) -- ([yshift=0.5cm]bisM6.north) -| ([yshift=0.25cm]bisM3.north) node[pos=0.25, above, movelabel] {4};
% Arrow for 6: goes below
\draw[movearrow] ([yshift=-0.25cm]bisM7.south) -- ([yshift=-0.5cm]bisM7.south) -| ([yshift=-0.25cm]bisM2.south) node[pos=0.25, below, movelabel] {6};
...
\node[font=\scriptsize\bfseries, black, anchor=west] at (\labelx, \rowA + 0.25) {BIS};
\draw[arrow] (\xI + \arrayw + \arrowgap, \rowB) -- (\xM - \arrowgap, \rowB);
\node[font=\scriptsize\bfseries, black, anchor=west] at (\labelx, \rowB + 0.25) {ESM};
\draw[arrow] (\xI + \arrayw + \arrowgap, \rowB) -- (2.8, \rowB) -- (2.8, \rowC) -- (\xM - \arrowgap, \rowC);
\node[font=\scriptsize\bfseries, black, anchor=west] at (\labelx, \rowC + 0.25) {DeltaSort};
...
\node[black, anchor=west] at (7.6, \rowA + 0.25) {\scriptsize\bfseries 19 {\tiny moves}};
\draw[arrow] (\xM + \arrayw + 0.1, \rowB) -- (\xO - \arrowgap, \rowB);
\node[black, anchor=west] at (7.6, \rowB + 0.25) {\scriptsize\bfseries 11 {\tiny moves}};
\draw[arrow] (\xM + \arrayw + 0.1, \rowC) -- (8.8, \rowC) -- (8.8, \rowB) -- (\xO - \arrowgap, \rowB);
\node[black, anchor=west] at (7.6, \rowC + 0.25) {\scriptsize\bfseries 9 {\tiny moves}};

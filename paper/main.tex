...
$O(n \log n)$ time or perform $k$ independent binary insertions, which can produce
incorrect results when dirty elements need to ``cross'' each other.

We present \emph{DeltaSort}, a coordinated repair algorithm that exploits knowledge of
which indices changed. DeltaSort's key insight is a \textbf{pre-sorting phase} that
establishes monotonicity among dirty values, eliminating crossing conflicts. Combined
with \textbf{direction-aware processing} and \textbf{bounded search ranges}, DeltaSort
achieves $O(k \log n)$ comparisons---optimal for comparison-based algorithms.

We prove correctness via loop invariants and establish tight bounds: each clean element
shifts at most $\min(k, M_c)$ times, where $M_c$ is the number of dirty elements whose
final positions ``pass over'' position $c$. This matches the information-theoretic minimum---no
algorithm can do better. Experiments across array sizes from 10K to 500K elements show
DeltaSort achieves 1.2--1.8$\times$ speedup over binary insertion for $20 \leq k \leq 200$,
with the gap widening for expensive comparators.
\end{abstract}
...
\paragraph{The Problem with Independent Repairs.}
The naive approach---$k$ independent binary insertions---has two issues:

\begin{enumerate}
  \item \textbf{Correctness}: If dirty element $A$ must move right past dirty element $B$'s
        current position, but $B$ must move left past $A$'s current position, the repair
        order matters. Incorrect ordering yields incorrect results.
  
  \item \textbf{Efficiency}: Each insertion searches the entire array ($O(\log n)$) and may
        shift $O(n)$ elements. With $k$ insertions, worst-case movement is $O(kn)$.
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{DeltaSort Algorithm} (\S\ref{sec:algorithm}): A three-phase algorithm that
        coordinates repairs through pre-sorting, direction classification, and bounded searches.
  ...
  \item \textbf{Tight Movement Bounds} (\S\ref{sec:movement}): We prove each clean element
        shifts exactly the minimum necessary times---no algorithm can do better.
  
  \item \textbf{Comparison Optimality} (\S\ref{sec:complexity}): DeltaSort uses $O(k \log n)$
        comparisons, matching the information-theoretic lower bound.
  
  \item \textbf{Experimental Validation} (\S\ref{sec:experiments}): Evaluation across array
        sizes, element types, and comparator costs.
\end{enumerate}
...
indices, and (3) correctness guarantees that independent repairs lack.
...
\label{def:crossing}
Two dirty indices $d_i, d_j$ with $d_i < d_j$ exhibit \emph{crossing} if their final
sorted positions satisfy $\pi(d_j) < \pi(d_i)$, i.e., the element at the higher index
belongs at a lower position than the element at the lower index.
\end{definition}
...
For each $d \in D$: binary search for correct position, reinsert.
Cost: $O(k \log n)$ comparisons, $O(kn)$ worst-case movement.
\textbf{Correctness issue}: May fail when dirty elements cross (order-dependent).
...
dirty values to $\{2,8\}$ and writes back, eliminating the crossing conflict.}
\label{fig:algorithm}
\end{figure}

\subsection{Why Phase 1 Matters: Eliminating Crossings}

\begin{example}[Crossing Conflict]
\label{ex:crossing}
...
\begin{itemize}
  \item Moving 8 first: 8 goes to position 4, yielding $[1, 5, 2, 9, 8]$
  \item Then moving 2: 2 goes to position 1, yielding $[1, 2, 5, 9, 8]$ --- \textbf{WRONG!}
\end{itemize}
The error occurs because we determined 8's target position before 2 moved, but 2's
movement invalidated that calculation.
\end{example}

Phase 1 prevents this: sort $\{8, 2\} \to \{2, 8\}$ and write to positions $\{1, 3\}$,
yielding $[1, 2, 5, 8, 9]$. Now both dirty elements are in their correct relative order.

\begin{lemma}[Phase 1 Eliminates Crossings]
\label{lem:no-crossing}
After Phase 1, for any dirty indices $d_i < d_j$, we have $\texttt{cmp}(A[d_i], A[d_j]) \leq 0$.
Consequently, no two dirty elements need to cross during repair.
\end{lemma}
...
For crossing to occur, we would need the value at $d_i$ to have a final position to the
right of the value at $d_j$'s final position. But since $A[d_i] \leq A[d_j]$, in any
sorted output, the position of $A[d_i]$ is at most the position of $A[d_j]$. Since
$d_i < d_j$, if both need to move in the same direction, they maintain order; they
cannot cross.
\end{proof}
...
Unlike an earlier incorrect claim, clean elements \emph{can} shift multiple times.
...
\toprule
Algorithm & Comparisons & Movement & Space & Always Correct? \\
\midrule
Native Sort & $O(n \log n)$ & $O(n \log n)$ & $O(n)$ & Yes \\
Binary Insertion & $O(k \log n)$ & $O(kn)$ & $O(1)$ & No* \\
Extract-Sort-Merge & $O(k \log k + n)$ & $O(n)$ & $O(n)$ & Yes \\
\textbf{DeltaSort} & $O(k \log n)$ & $O(kn)$** & $O(k)$ & Yes \\
\bottomrule
\end{tabular}

\vspace{0.3em}
{\small *Fails when dirty elements cross without pre-sorting.}\\
...
\begin{itemize}
  ...
  \item \textbf{BI}: Binary Insertion with pre-sorted dirty values (for correctness)
...
Both have $O(kn)$ worst-case movement. The practical difference:
\begin{enumerate}
  \item \textbf{Bounded search}: DS searches smaller ranges
  \item \textbf{Cache locality}: Processing in index order improves cache behavior
  \item \textbf{No redundant work}: Pre-sorting handles crossings once, not per-insertion
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{Correctness}: Automatic handling of crossing conflicts via Phase 1
...
\begin{enumerate}
  \item \textbf{Pre-sorting phase} that eliminates crossing conflicts, enabling correct
        repair without order dependence
...
We explicitly corrected an earlier false claim about single-shift guarantees: clean
elements can shift multiple times when multiple dirty elements pass over them. This is
unavoidable for any in-place incremental approach.

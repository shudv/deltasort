...
\[
O(k \log k) + O(k \log (n/k))
= O\!\bigl(k(\log k + \log (n/k))\bigr)
= O(k \log n).
\]
\end{proof}
...
\begin{itemize}
    ...
    \item Update-aware algorithms: \textbf{Binary-Insertion-Sort (BIS)}, \textbf{Extract-Sort-Merge (ESM)}
\end{itemize}

Together, these provide strong baselines for comparison across the full range of update batch sizes ($k$). 

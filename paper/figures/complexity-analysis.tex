...
% Comparisons: O(k log(n√k)) → normalized = comparisons / (k log(n√k)) should be constant
\begin{figure}[t]
\centering
...
\begin{axis}[
    ...
    title={\textbf{(a) Normalized Movement (Phase~2)}},
]
...
\end{tikzpicture}
\end{minipage}
\caption{Empirical validation of DeltaSort's complexity bounds. Each line represents a different array size~$n$. (a)~Movement normalized by $n\sqrt{k}$ converges to $\approx 0.4$, confirming $O(n\sqrt{k})$. (b)~Comparisons normalized by $k\log(n\sqrt{k})$ converge to $\approx 1.4$, confirming $O(k\log n)$. Convergence across scales validates the asymptotic bounds.}

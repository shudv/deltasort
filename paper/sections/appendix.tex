...
\begin{enumerate}
  ...
  \item ESM is $\sim 30$--$40\%$ faster than FullSort (\texttt{Array.prototype.sort}) for $k$ up to $\approx 40\%$. This indicates that \emph{even in managed runtimes, we can leverage information of updated indices to unlock better performance}.
\end{enumerate}

The key takeaway is that \emph{DeltaSort's performance benefits rely on a predictable movement cost model}, which holds in low-level, unmanaged environments (e.g., Rust) but not in V8. Hence in V8, a valid hybrid strategy can be: use DeltaSort for $k \ll n$, ESM for $k \lesssim 40\%$, \texttt{Array.prototype.sort} for $k \gtrsim 40\%$.

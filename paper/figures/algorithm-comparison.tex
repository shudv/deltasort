...
\node[black, anchor=west] at (7.8, \rowB + 0.25) {\scriptsize\bfseries 14, 11};
...
\end{tikzpicture}
\caption{Movement and space comparison for all algorithms for sorting 3 updated values (yellow). ESM and DeltaSort are assumed to sort updated values using insertion sort. BIS first uses 8 moves to shift updated values to the end (since insertion requires a fully sorted prefix), then 10 (0+4+6) moves to insert each updated value into the sorted prefix. ESM uses 3+3 moves to extract and sort the updated values, then writes all 8 values to produce the merged output. DeltaSort uses 3+3+3 moves to collect sort and write the updated values back to the array, then just 6 (2+2+2) moves to place each value correctly within its \emph{segment} (see~\defref{def:segment}), \textbf{without having to form a fully sorted prefix upfront} like BIS.}

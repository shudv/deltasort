...
\begin{itemize}
  \item \textbf{LEFT ($L$)}: Value is ordered incorrectly to its left neighbour: $i > 0 \text{ and } \texttt{cmp}(A[i-1], A[i]) > 0$
  \item \textbf{RIGHT ($R$)}: Value is either already sorted correctly w.r.t its neighbours or ordered incorrectly to its right neighbour: $i = 0 \text{ or } \texttt{cmp}(A[i-1], A[i]) \le 0$
\end{itemize}
\end{definition}
...
The key insight behind DeltaSort is that segments created after Phase~1 can be fixed locally. Unlike Binary-Insertion-Sort, where each insertion can be at any point in the array, an updated value in a segment cannot cross its segment boundary. We formally prove this property next.

...
\begin{itemize}
  ...
  \item \textbf{BIS}: In-place binary insertion sort that partitions dirty elements to the end while preserving clean element order, then inserts each dirty element via binary search and rotation. Uses $O(1)$ auxiliary space.

  \item \textbf{ESM}: Extract dirty values into a buffer, sort, compact clean elements in-place, and merge backwards from the right end. Uses $O(k)$ auxiliary space. Internally uses Rust's \texttt{sort\_unstable\_by} for sorting.

  \item \textbf{DeltaSort}: Standard implementation as described in \secref{sec:algorithm}. Sorts updated values in-place using indirect heapsort, then fixes violations using binary search and rotation. Uses $O(1)$ auxiliary space beyond the input index list.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{DeltaSort is the fastest algorithm for} $k \lesssim 1\%$. For example, at $k = 0.1\%$, DeltaSort is $\sim$10$\times$ faster than FullSort, $\sim$2$\times$ faster than ESM and $\sim$27$\times$ faster than BIS. Even though $1\%$ is small in absolute terms, it aligns very well with practical workloads where updates usually affect only a small percentage of the full dataset.

  \item ESM is the fastest algorithm across most of the intermediate range ($1\% \lesssim k \lesssim 80\%$). \textbf{DeltaSort provides a space-efficient alternative to ESM for} $k \lesssim 10\%$: it uses $O(1)$ auxiliary space compared to ESM's $O(k)$, while still beating FullSort by $\sim$2$\times$.
  
  \item Crossover threshold trend is consistent and largely stable across all scales (DeltaSort $\sim$8--22\%, ESM $\sim$60--95\%), indicating that \textbf{the above observations hold across scales}. BIS has a very low threshold due to its quadratic data movement cost, whereas ESM has the highest threshold due to its efficient linear merge pass. DeltaSort exhibits an intermediate threshold, in line with its intermediate time complexity.
\end{enumerate}

These observations suggest that, much like hybrid blind sorting algorithms (e.g., TimSort~\cite{timsort}, DriftSort~\cite{driftsort}), it would be beneficial to construct \emph{adaptive update-aware} strategies. As an example, for the Rust implementation evaluated here, an adaptive strategy for an environment that requires fast execution without excessive space usage could be: use DeltaSort for $k \lesssim 10\%$, ESM for $10\% \lesssim k \lesssim 80\%$, and FullSort for $k \gtrsim 80\%$. The optimal cut-off points for a scenario would depend on several factors like memory availability, distribution of update sizes, comparator cost etc. For example, as the comparator cost grows, DeltaSort preserves its advantage over BIS because both have similar comparison count: $O(k \log n)$, while widening its gap relative to ESM, which has substantially higher comparison overhead. Hence, the \emph{crossover thresholds would shift in favor of DeltaSort}, expanding the range of update sizes for which it is the preferred strategy.
...
\begin{enumerate}
  ...
  \item DeltaSort is faster than FullSort by $\sim$8$\times$ for $k\lessapprox0.01\%$, and ESM is $\sim$1.5$\times$ faster than FullSort for $k$ up to $\approx$45\%, indicating that even in managed runtimes, \textbf{exposing updated indices can unlock better incremental sort performance}.
\end{enumerate}

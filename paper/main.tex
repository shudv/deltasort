...
We first analyze the expected total data movement during Phase~2 under a bounded-range update model where updated values are drawn uniformly from a fixed range. This ensures that there is no inherent structure in the updates that DeltaSort could exploit beyond segmentation.

\begin{remark}
   We could choose other models such as unbounded-range model (values can be in any arbitrary range) or perturbation model (where values change based on a particular random variance from their existing value). Both these models lead to better expected movement bounds as compared to bounded-range model hence we chose the stricter model for analysis. Given these alternate models have legitimate practical relevance, we will explore them more formally in future work. 
\end{remark}

\begin{theorem}[Expected Movement Bound]
\label{thm:movement-bound}
Under a bounded-range update model, for an array of size $n$ with $k$ updates, the expected total data movement during DeltaSort's repair phase is $O(n)$ (independent of $k$).
\end{theorem}

...
\relatedversiondetails[linktext={Source code}]{Repository}{https://github.com/shubd3/deltasort}
...
\supplement{Source code and benchmark data are available at \url{https://github.com/shubd3/deltasort} under the MIT license. Benchmark results are reproducible using the instructions in the repository README.}
...
\subsection{Key Insight: Segmentation Enables Localized Repair}
\label{sec:insight}
...
\begin{proof}
The only violations in the array after Phase~1 are at updated indices. Phase~2 processes each updated index exactly once. By \cref{lem:fix-invariant}, each fix resolves a violation without introducing new ones. After all fixes, no violations remain, so the array is sorted.
\end{proof}
...
While worst-case movement is $O(kn)$, the segmentation created by Phase~1 tends to reduce movement in the average case. Empirical results in \cref{sec:experiments} demonstrate substantial speedups, validating that segmentation (\cref{lem:confinement}) effectively reduces movement in practice. We will analyze average-case movement more rigorously in future work.
\end{remark}

\cref{tab:complexity} compares algorithm complexity with the standard baseline approaches used in the experiments:
\begin{itemize}
...
\begin{table}[t]
\centering
\caption{Algorithm complexity comparison. *DeltaSort worst-case movement is $O(kn)$; average case is empirically much smaller.}
...
All experiments were conducted using a Rust implementation of DeltaSort~\cite{deltasort-repo} compiled with \texttt{rustc 1.83.0} using release mode optimizations (\texttt{-{}-release}). Benchmarks were run on an Apple M3 Pro MacBook Pro with 18GB RAM running macOS 15.1 (Sequoia). The machine was kept idle during benchmarks to minimize interference from background processes. CPU frequency scaling was disabled for consistent measurements.

\paragraph{Dataset.} We use a synthetic dataset of user objects with composite keys consisting of (country, age, name) fields, sorted lexicographically. This simulates real-world sorted data with multi-field comparators. For each benchmark configuration, we:
\begin{enumerate}
    \item Generate a sorted base array of size $n$
    \item Randomly select $k$ distinct indices to update
    \item Mutate the selected elements with random values
    \item Run the sorting algorithm and measure execution time and comparator invocations
\end{enumerate}

\paragraph{Methodology.} Each measurement consists of 100 iterations with fresh random mutations per iteration to capture variance. We report mean values with 95\% confidence intervals computed using $\bar{x} \pm 1.96 \cdot \sigma/\sqrt{n}$. Warm-up runs (5 iterations) precede each benchmark to stabilize JIT and cache behavior.

\paragraph{Baselines.} DeltaSort is compared against three baselines: NativeSort (Rust's \texttt{sort\_by}, which uses PDQSort~\cite{peters2021pdqsort}), Binary-Insertion-Sort (BIS), and Extract-Sort-Merge (ESM).

\subsection{Correctness}
Correctness is formally proven in \cref{thm:correctness} and also verified by an extensive set of randomized unit tests~\cite{deltasort-repo} across various scales and update sizes. The test routine generates a sorted base array of size $n$, applies $k$ random updates at random indices, runs DeltaSort, and asserts that the final array is sorted and contains all original values with updated values. All tests pass successfully.

\subsection{Execution Time}

\cref{fig:rust-execution-time} shows execution time (in microseconds) for $n = 50$K values as a function of updated count $k$. DeltaSort consistently outperforms all alternatives up to approximately $k = 16.5$K (crossover point), achieving significant speedups of $4$--$20\times$ over NativeSort in the intermediate range. Also note the orders-of-magnitude gap between DeltaSort and the baseline incremental algorithms (BIS and ESM) across the full range of $k$ values.
...
\end{axis}
\end{tikzpicture}
\caption{Comparator invocation count for $n = 50$K. DeltaSort and BIS both achieve $O(k \log n)$ comparisons, while NativeSort uses $O(n \log n)$ regardless of $k$, and ESM uses $O(k \log k + n)$. The 10--40\% higher comparison counts for DeltaSort vs BIS confirm that movement confinement (\cref{lem:confinement}) is the major factor in DeltaSort speedup.\benchmarknote}
...
\node[font=\scriptsize\bfseries, text=purple!70!black, anchor=south west] at (axis cs:55000, 34) {$n{=}50$K (\cref{fig:rust-execution-time})};
...
Our evaluation has several limitations that may affect generalizability:

\begin{itemize}
    \item \textbf{Synthetic data:} We use synthetic user objects with composite keys. Real-world data may have different value distributions and comparator costs.
    \item \textbf{Random updates:} Updates are uniformly distributed across indices. Workloads with clustered or localized updates may behave differently.
    \item \textbf{Single platform:} Results are from Apple Silicon (M3 Pro). Performance characteristics may differ on x86-64 or other architectures due to cache hierarchy and branch prediction differences.
    \item \textbf{Single language:} The Rust implementation benefits from zero-cost abstractions. Implementations in garbage-collected languages may show different behavior (see \cref{sec:managed}).
\end{itemize}

\subsection{Performance in Managed Execution Environments}
\label{sec:managed}

DeltaSort was also implemented in JavaScript~\cite{deltasort-repo} and benchmarked on the V8 engine to evaluate behavior in managed runtimes. Initial results indicate that DeltaSort outperforms NativeSort for some workloads, but the gains are more modest and the crossover point occurs at smaller update sizes. This behavior appears to be driven primarily by the performance characteristics of the native JavaScript sort, which is highly optimized and handles nearly sorted inputs exceptionally well. As a result, the relative advantage of segmented repair is reduced in this environment compared to the Rust implementation.

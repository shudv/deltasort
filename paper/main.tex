...
\begin{proof}
Let $S$ be a segment with R indices $R_1, \ldots, R_r$ followed by L indices $L_1, \ldots, L_l$ (where $r \ge 0$ and $l \ge 0$, with $r + l \ge 1$). After Phase~1, updated values are monotonically ordered by index, so $A[R_1] < \cdots < A[R_r] < A[L_1] < \cdots < A[L_l]$.
...
\begin{enumerate}
  ...
  \item L values move leftward but cannot pass the last R value $R_r$ (if it exists) or the segment boundary, since $A[R_r] < A[L_j]$ for all $j$.
\end{enumerate}
